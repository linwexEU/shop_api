...
from src.organization_rates.schema import SOrganizationRate, SOrganizationRateAdd, SOrganizationRateAddResponse, SOrganizationRatesFilters, SOrganizationRatesModel, SOrganizationRatesResponse
from src.auth.dependencies import CurrentUserDep, OwnerUserDep
from src.organizations.schema import SOrganizationAdd, SOrganizationAddResponse, SOrganizationDeleteResponse, SOrganizationResponse, SOrganizationsFilters, SOrganizationsModel
from src.logger import config_logger
from src.utils.dependency import OrganizationRatesInteractionDep, OrganizationRatesServiceDep, OrganizationsServiceDep, UsersServiceDep
from src.utils.exception import InteractionException, NotYourOrganizationException, OrganizationDeletedException, YouCantRateYourselfException
...
class OrganizationFlow: 
    ...
    async def get_organization_flow(self, organization_id: int) -> SOrganizationResponse: 
        try:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Organization(Id={organization_id}) was not found")

    async def add_organization_flow(self, data: SOrganizationAdd, owner: OwnerUserDep) -> SOrganizationAddResponse: 
        try: 
    ...
    async def get_organization_rates_flow(self, organization_id: int) -> SOrganizationRatesResponse: 
        try:
    ...
    async def add_rate_to_organization_flow(self, organization_id: int, data: SOrganizationRateAdd, current_user: CurrentUserDep) -> SOrganizationRateAddResponse: 
    ...
    async def delete_organization_flow(self, organization_id: int, owner: OwnerUserDep) -> SOrganizationDeleteResponse:
        ...
        try:
            await self.org_service.add_delete_utc(organization_id, datetime.now(timezone.utc))
    ...
    async def get_cached_organization(self, organization_id: int): 
        organization = await self.org_service.get_by_filters(SOrganizationsFilters(id=organization_id))
        return SOrganizationResponse.to_dict(organization)

    @cache(120) 
    async def get_cached_rates(self, organization_id: int): 
        rates = await self.org_rates_service.get_by_filters(SOrganizationRatesFilters(organization_id=organization_id), False) 
