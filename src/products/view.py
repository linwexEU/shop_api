...
from src.product_rates.schema import (SRateDislikeResponse, SRateLikeResponse,
                                      SRatesModel, SRatesModelResponse,
                                      SRatesResult)
from src.products.flow import ProductsFlow
from src.products.schema import (Pagination, SProductAddResponse,
                                 SProductImage, SProductsResult,
                                 SProductsSearch)
...
@router.post(
    "/add/{organization_id}",
    response_model=SProductAddResponse,
...
@router.post(
    "/{product_id}/rates",
    response_model=SRatesModelResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.post(
    "/rates/{rate_id}/replies",
    response_model=SProductRateRepliesModelResponse,
    status_code=status.HTTP_201_CREATED,
)
async def add_reply_for_rate(
    rate_id: int,
