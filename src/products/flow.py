...
from src.product_rate_replies.schema import SProductRateReplies, SProductRateRepliesFilters, SProductRateRepliesModel, SProductRateRepliesModelResponse, SProductRateRepliesResult
from src.product_rates.schema import SProductRatesFilters, SProductRatesModel, SRateDislikeResponse, SRateLikeResponse, SRates, SRatesModel, SRatesModelResponse, SRatesResult
from src.products.schema import Pagination, SProductAddResponse, SProductImage, SProductsResult, SProductFilters, SProducts, SProductsSearch
from src.utils.dependency import ProductRateRepliesServiceDep, ProductRatesServiceDep, ProductsServiceDep, ProductRatesInteractionDep, UsersServiceDep
from src.models.enums import ActEnum, CategoriesEnum
from src.logger import config_logger
from src.utils.exception import NotExcelFileException, NotYourOrganizationException
...
class ProductsFlow: 
    ...
    async def get_products_by_category(self, pagination: Pagination, category: CategoriesEnum) -> SProductsResult: 
        try:
            products = await self.get_cached_products(category)
            
            # Set paginatrion
            pagination_result = products[pagination.offset:pagination.offset + pagination.limit]
    ...
    async def add_product_flow(self, organization_id: int, excel_file: UploadFile, owner: OwnerUserDep) -> SProductAddResponse: 
    ...
    async def add_product_rate_flow(self, product_id: int, data: SRatesModel) -> SRatesModelResponse:
        try:
    ...
    async def get_product_rate_replies_flow(self, rate_id: int) -> SProductRateRepliesResult: 
        try:
            ...
            if isinstance(ex, SQLAlchemyError): 
                ...
                logger.error("Unknown error: %s" % ex) 
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Replies for Rate(Id={rate_id}) were not found")
    
    async def add_product_rate_reply_flow(self, rate_id: int, data: SProductRateRepliesModel) -> SProductRateRepliesModelResponse: 
        try: 
    ...
    async def get_product_image(self, product_id: int) -> SProductImage: 
        try:
            product = await self.products_service.get_by_filters(SProductFilters(id=product_id))
    ...
    async def like_rate_flow(self, rate_id: int) -> SRateLikeResponse:
        ...
        try: 
            # Add like
            rate_id = await self.product_rates_service.like_rate(rate_id, self.current_user.id)
            if rate_id is None: 
            ...
            if isinstance(ex, SQLAlchemyError): 
                ...
                logger.error("Unknown error: %s" % ex) 
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Rate(Id={rate_id}) wan't found") 
    ...
    async def dislike_rate_flow(self, rate_id: int) -> SRateDislikeResponse:
        ...
        try: 
            # Add dislike
            rate_id = await self.product_rates_service.dislike_rate(rate_id, self.current_user.id)
            if rate_id is None: 
                raise SQLAlchemyError
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Rate(Id={rate_id}) wasn't found")
    ...
    async def get_cached_products(self, category: CategoriesEnum): 
        query_products = await self.products_service.get_by_filters(SProductFilters(category=category), False)
        return [SProducts.to_dict(p) for p in query_products] 

    @cache(120) 
    async def get_cached_products_from_search(self, search: SProductsSearch, category: CategoriesEnum): 
        query_products = await self.products_service.get_products_by_search(search, category)
        if query_products:
            return [SProducts.to_dict(p) for p in query_products]  
        return []
    ...
    async def get_cached_product_rates(self, product_id: int): 
        rates = await self.product_rates_service.get_by_filters(SProductRatesFilters(product_id=product_id), False)
        return [SRates.to_dict(r) for r in rates]
    
    @cache(120) 
    async def get_cached_product_rate_replies(self, rate_id: int): 
        replies = await self.product_rate_replies_service.get_by_filters(SProductRateRepliesFilters(rate_id=rate_id), False)
