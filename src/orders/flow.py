...
from src.orders.schema import (SOrderResult, SOrders, SOrdersFilters,
                               SOrdersModel, SOrdersPayload)
    ...
    async def change_state_flow(self, uuid: str, state: OrdersStateChangerEnum) -> None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Order(Uuid={uuid}) was not found",
            )
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Invalid state transition from '{order.state.value}'",
            )
