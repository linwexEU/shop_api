...
from src.notifications.schema import SNotificationsFilters, SNotificationsModel, SNotificationsPayload, SNotificationsReadResponse
...
class NotificationsFlow: 
    ...
    async def get_notifications_flow(self) -> SNotificationsPayload: 
        try: 
            ...
            return SNotificationsPayload(
                NewCount=len(list(filter(lambda x: x.state == NotificationsStateEnum.New, data))), 
                Data=data
            )
    ...
    async def mark_as_read_flow(self, notification_id: int) -> SNotificationsReadResponse: 
        ...
        try: 
            ...
            if isinstance(ex, SQLAlchemyError): 
                ...
                logger.error("Unknown error: %s" % ex) 
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Notification(Id={notification_id}) was not found")

    async def check_access(self, notification_id: int): 
        try:
            notification = await self.notifications_service.get_by_filters(SNotificationsFilters(id=notification_id)) 
        except Exception as ex: 
            if isinstance(ex, SQLAlchemyError): 
                logger.error("Db error: %s" % ex) 
            else: 
