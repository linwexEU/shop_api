...
import aiohttp
from fastapi import HTTPException, Response, status, Request, Response
from authlib.integrations.starlette_client import OAuthError
from pydantic import EmailStr
from sqlalchemy.exc import SQLAlchemyError

from src.models.enums import UsersRoleEnum
from src.auth.auth import authenticate_user, create_access_token, get_hash_password
from src.config import settings
from src.logger import config_logger
from src.users.schema import SUsersAuth, SUsersAuthResponse, SUsersModel, SUsersRegistration, SUsersRegistrationResponse
from src.utils.decorator import async_retry
from src.utils.dependency import UsersServiceDep
from src.utils.exception import CredentialsException, InvalidAdminToken, InvalidEmailException
...
class AuthFlow: 
    ...
    async def register_flow(self, data: SUsersRegistration) -> SUsersRegistrationResponse: 
        # Get password hash
        ...
        if data.admin_token and data.role == UsersRoleEnum.Admin:
            if not secrets.compare_digest(data.admin_token.encode(), settings.ADMIN_TOKEN.encode()): 
                raise InvalidAdminToken
        
        try: 
            user_id = await self.users_service.add(SUsersModel.from_orm(data, hashed_password))
    ...
    async def authenticate_flow(self, response: Response, data: SUsersAuth) -> SUsersAuthResponse: 
        try:
    ...
    async def google_oauth_callback_flow(self, request: Request, response: Response) -> SUsersAuthResponse:
        from src.auth.view import oauth
        ...
        user = await self.users_service.get_or_create(
            SUsersModel(
                ...
                hashed_password=get_hash_password("".join(random.sample(string.ascii_letters + string.digits, 8)))
            )
        )
