...
async def get_token(request: Request): 
    token = request.cookies.get("access_token") if request.cookies.get("access_token") else request.headers.get("Authorization")
    if token is None: 
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED) 
    return token 


async def get_current_user(token: Annotated[str, Depends(get_token)], users_service: UsersServiceDep) -> SUserAuthModel: 
    try:
