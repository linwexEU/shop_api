...
class GenerateExcel:
    ...
    def add_headings(self, headings: list[str]) -> None:
        ...
        for col in range(1, len(headings) + 1):
            ...
            self.ws[get_column_letter(col) + "1"].font = Font(
                bold=True, color="00FFFFFF"
            )
            self.ws[get_column_letter(col) + "1"].border = self.border
            self.ws[get_column_letter(col) + "1"].fill = PatternFill(
                start_color="0057bc5f", end_color="0057bc5f", fill_type="solid"
            )
    ...
    def generate_file(
        self, filename: str, headings: list[str], data: list[dict[str, str | int]]
    ):
    ...
    def generate_file_buffer(
        self, headings: list[str], data: list[dict[str, str | int]]
    ):
        self.add_headings(headings)
        self.add_data(len(headings), data)
