...
class RozetkaParser:
    ...
    def __init__(
        self, url: str, category_type: CategoriesEnum, store: Literal["db", "excel"]
    ) -> None:
        self.url = url
    ...
    async def create_test_organization() -> int:
        ...
        has_created = await user_services_db.get_by_filters(
            SUsersFilters(email="test@dot.com")
        )
        ...
        if not has_created:
            ...
            user = await user_services_db.get_by_filters(
                SUsersFilters(email="test@dot.com")
            )
            user_id = user.id

            organization = await organizations_service_db.get_by_filters(
                SOrganizationsFilters(user_id=user_id)
            )
    ...
    async def parse_pages(self) -> None:
        ...
        for page in range(1, pagination + 1):
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.url.replace("page=1", f"page={page}"), headers=self.headers
                ) as response:
                    html = await response.text()
                    soup = BeautifulSoup(html, "lxml")
                    ...
                    for product in products:
                        try:
                            title = product.find(
                                "a", class_="tile-title black-link text-base"
                            ).text.strip()
                            ...
                            if price is None:
                                price = (
                                    product.find("div", class_="price color-red")
                                    .text.replace("\xa0", "")
                                    .strip(" ₴")
                                )
                            price = float(price)
        ...
        if self.store == "excel":
            try:
                ...
                excel_generator.generate_file(
                    "monitors",
                    ["title", "category", "price", "image", "organization_id"],
                    excel_data,
                )
            except Exception as ex:
...
def get_url(category: str) -> tuple[str, CategoriesEnum]:
    match category:
        case "Computers":
            return settings.COMPUTERS_URL, CategoriesEnum.Computers  # parsed
        case "Phones":
            return settings.PHONES_URL, CategoriesEnum.Phones  # parsed
        case "Monitors":
            return settings.MONITORS_URL, CategoriesEnum.Monitors  # parsed
        case "Lego":
            return settings.LEGOS_URL, CategoriesEnum.Legos  # parsed
        case "Books":
            return settings.BOOKS_URL, CategoriesEnum.Books  # parsed
        case "Keyboards":
            return settings.KEYBOARDS_URL, CategoriesEnum.Keyboards  # parsed
        case "Mouses":
            return settings.MOUSES_URL, CategoriesEnum.Mouses  # parsed
        case "Electronics":
            return settings.ELECTRONICS_URL, CategoriesEnum.Electronics  # parsed
